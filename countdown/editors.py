...
def get_coordinates(x_coordinate, width, y_coordinate):
    return x_coordinate - (width - 4) / 2, y_coordinate


def create_cover(
        countdown, cover_orginal_path, cover_final_path, 
        font_path, color_rgb, color_size, x_coords, y_coord):
    ...
    coords = [get_coordinates(*c, y_coord) for c in zip(x_coords, widths)]
